...
class MyListsController extends AppController {
    ...
    public function admin_add($list_id = null) {
        ...
        #Fetch Vibes
        $vibes = $this->Vibe->find('all');
        $regions = $this->Region->find('all');
        $this->set(compact('categories', 'vibes', "regions"));
        
        $this->set("user", $this->User->find("list", array("conditions" => array("User.status" => 1, "User.role_id" => 4), "fields" => array("User.id", "User.username"))));
        if ($this->data) { 
            if ($this->MyList->save($this->data)) {
                if (!empty($this->data["MyList"]["id"])) {
                    echo $my_list_id = $this->data["MyList"]["id"];
                    $not_save_email = "";
                } else {
                ...
                // now save category
                $MyEventCategory = $this->data["ListCategory"]["category_id"];
               
                $this->loadModel("ListCategory");
                $this->ListCategory->deleteAll(array('ListCategory.my_list_id' => $my_list_id), false);
                foreach ($MyEventCategory as $tc) {
                    #pr($tc);die;

                    if ($tc != 0) {
                        $save_tc["ListCategory"]["id"] = "";
                        $save_tc["ListCategory"]["category_id"] = $tc;
                        $save_tc["ListCategory"]["my_list_id"] = $my_list_id;
                        $this->ListCategory->save($save_tc);
                    }
                }
                # now save vibes
                $MyListVibe = $this->data["ListVibe"]["vibe_id"];
                $this->loadModel("ListVibe");
                $this->ListVibe->deleteAll(array('ListVibe.my_list_id' => $my_list_id), false);
                foreach ($MyListVibe as $ev) {

                    if ($ev != 0) {
                        $save_ev["ListVibe"]["id"] = "";
                        $save_ev["ListVibe"]["vibe_id"] = $ev;
                        $save_ev["ListVibe"]["my_list_id"] = $my_list_id;
                        $this->ListVibe->save($save_ev);
                      
                    }
                }
                # now save emails
                $MyListEmail = $this->data["ListEmail"]["email"]; #pr($EventVibe);die;
             
                $this->loadModel("ListEmail");
                // for not save email while editing
                if (!isset($not_save_email)) {
                    foreach ($MyListEmail as $em) {

                        $save_em["ListEmail"]["id"] = "";
                        $save_em["ListEmail"]["email"] = $em;
                        $save_em["ListEmail"]["my_list_id"] = $my_list_id;
                        $this->ListEmail->save($save_em);
                     
                    }
                }
                # now save region
                $MyListRegion = $this->data["ListRegion"]["region_id"]; #pr($EventVibe);die;
              
                $this->loadModel("ListRegion");
                $this->ListRegion->deleteAll(array('ListRegion.my_list_id' => $my_list_id), false);
                foreach ($MyListRegion as $er) {

                    if (trim($er) != 0) { 
                        $save_er["ListRegion"]["id"] = "";
                        $save_er["ListRegion"]["region_id"] = $er;
                        $save_er["ListRegion"]["my_list_id"] = $my_list_id;
                        $this->ListRegion->save($save_er);
                     
                    }
                }
                $this->Session->setFlash("List Saved", "default", array("class" => "green"));
                $this->redirect(array("controller" => "MyLists", "action" => "list"));
            }
        }
        if ($list_id) {
            $this->request->data = $this->MyList->findById(base64_decode($list_id));
        }
    }
    ...
    function lists() {
        ...
        $this->paginate = array('conditions' => "", 'limit' => 4, 'order' => array('MyList.id' => 'ASC'), 'recursive' => 2);
    ...
    public function myList() {
        ...
        foreach ($mylists as $mylist) {
            ...
            $mylists[$i]["MyList"]["total_over_all"] = $total_over_all;
    ...
    public function import() {
        ...
        if ($this->data) {
           ...
           $contact_list_email = explode(",",htmlspecialchars($this->data["MyList"]["contact_list"]));
    ...
    public function admin_exportCsv($myList_id = null) {
        ...
        if ($myList_id) {
            ...
            $this->Session->setFlash("Direct access not allowed", "default", array("class" => "red"));
            $this->redirect(array("controller" => "MyLists", "action" => "listView"));
        }
    }
    ...
    public function admin_addOfferPrice($offerPriceId = NULL) {
        $this->layout = "admin/admin";
        $this->loadModel("OfferPrice");
        $lists = $this->MyList->find("list", array("conditions" => array("MyList.status" => 1), "fields" => array("MyList.id", "MyList.list_name")));
        $this->set("lists", $lists);
        if ($this->data) {
            if ($this->OfferPrice->save($this->data)) {
                $this->Session->setFlash("Offer price updated succeefully.", "default", array("class" => "green"));
