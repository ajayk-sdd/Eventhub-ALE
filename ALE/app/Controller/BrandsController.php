...
class BrandsController extends AppController {
    ...
    public function admin_list() {
        ...
        if ($this->data) {
            ...
            $this->paginate = array('conditions' => $conditions, 'limit' => 4, 'order' => array('Brand.id' => 'ASC'));
        }
    ...
    public function admin_add($brand_id = null) {
        ...
        $this->set("user", $this->User->find("list", array("conditions" => array("User.status" => 1, "User.role_id != " => 1), "fields" => array("User.id", "User.username"))));
        ...
        if ($this->data) {
            ...
            if ($this->Brand->save($tmp)) {
                ...
                $this->loadModel("User");
                $this->User->save($user);
                $this->Session->setFlash("Brand saved", "default", array("class" => "green"));
                $this->redirect(array("controller" => "Brands", "action" => "list"));
            }
        }
        if ($brand_id) {
    ...
    public function mySubcription() {
        ...
        if ($this->data) {
            ...
            $conditions = array("SuscribeBrand.user_id" => AuthComponent::User("id"));
            $this->paginate = array('conditions' => $conditions, 'limit' => 4, 'order' => array('SuscribeBrand.id' => 'ASC'));
        }
