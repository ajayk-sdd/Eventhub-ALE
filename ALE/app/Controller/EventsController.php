...
class EventsController extends AppController {
    ...
    function beforeFilter() {
        parent::beforeFilter();
        $this->Auth->allow('dayOfMounth', 'findNoOfDay', 'findWeekDay', 'all', 'findVenue', 'fetchCategoryFromEventful', 'reviewEvent', 'calendar', 'viewEvent', 'eventfulEvents', 'ticketGiveaway', 'ticketGiveawayDetail', 'ticketGiveawayThankYou', 'addattend', 'addattendmult', 'viewEventfulEvent', 'confirm', 'activate', 'totalCount');
    }
    ...
    function createEvent($id = null, $type = null) {
        ...
        #Fetch Vibes
        $vibes = $this->Vibe->find('list', array("conditions" => array("Vibe.status" => 1), "fields" => array("Vibe.id", "Vibe.name")));
        ...
        if ($this->data) {
            ...
            for ($i = 0; $i < count($tmp['Event']['ticket_price']); $i++) {
                $tmp['Event']['ticket_price'] = implode('|', $tmp['Event']['ticket_price']);
            }
            ...
            if ($this->Event->save($tmp)) {
                ...
                if (!empty($this->data['EventImage']['image_name'][0]['name'])) {
                ...
                $EventstarttimeF = $tmp["EventDate"]["start_timeF"];
                $EventendtimeF = $tmp["EventDate"]["end_timeF"];
                ...
                for ($i = 0; $i < count($Eventdate); $i++) {
                    ...
                    $tmp_date['EventDate']['start_time'] = $Eventstarttime[$i] . $EventstarttimeF[$i];
                    $tmp_date['EventDate']['end_time'] = $Eventendtime[$i] . $EventendtimeF[$i];
    ...
    public function admin_list() {
        ...
        $conditions = array("Event.event_from !=" => "eventful");
        ...
        if ($this->data) {
            ...
            $this->paginate = array('conditions' => $conditions, 'limit' => $this->data["Event"]["limit"], 'order' => array($this->data["Event"]["order"] => $this->data["Event"]["direction"]));
            $this->request->data = $this->data;
            $this->Session->write("event_list", $this->data);
        } else {
            $this->paginate = array('conditions' => $conditions, 'limit' => 10, 'order' => array('Event.id' => 'ASC'));
        }
    ...
    public function ticketGiveaway($event_id = null) {
        ...
        if ($this->data) {
            ...
            if (!empty($ticketPresent)) { //pr($ticketPresent);die;
    ...
    public function admin_addGiveaway($id = NULL) {
        ...
        if ($this->data) {
            if ($this->Giveaway->save($this->data)) {
                $this->Session->setFlash("Giveaway saved", "default", array("class" => "green"));
    ...
    public function admin_listGiveaway() {
        ...
        if ($this->data) {
            ...
            $this->paginate = array('conditions' => $conditions, 'limit' => 10, 'order' => array('Event.title' => 'ASC'));
        }
    ...
    public function fetch($email = null, $password = null, $provider = null) {
        $this->layout = false;
        $ch = curl_init();
        $url = "http://" . $_SERVER['HTTP_HOST'] . "/open/OpenInviter/example.php?email_box=$email&password_box=$password&provider_box=$provider";
