$(document).ready(function() {
    ...
    $('body').delegate(".featureImg", 'click', function() {

        var id = $(this).attr("id");
        var featureId =  id.split("_"); 
        
        $('#load_feature_' + id).show();
        var model = $(this).attr("dir");
        var rel = $(this).attr("rel");
        var status = $(this).attr("value");
        //alert(rel+"--"+model+"--"+status);
        jQuery.ajax({
            url: '/admin/Commons/changefeature/' + rel + '/' + status + '/' + model,
            success: function(data) {
                $('#load_feature_' + id).hide();
                if (data == 1) {
                    $("#feature_" + featureId[1]).attr({'src': '/img/admin/active.png', 'class': 'featureImg', 'id': id, 'rel': rel, 'value': 1, 'model': model});
                } else {
                    
                    $("#feature_" + featureId[1]).attr({'src': '/img/admin/inactive.png', 'class': 'featureImg', 'id': id, 'rel': rel, 'value': 0, 'model': model});
                }
            }
        });
