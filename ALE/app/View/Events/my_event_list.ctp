                            ...
                            <tbody>
                                <?php foreach ($events as $event): ?>
                                    <tr>
                                        ...
                                        <td><?php foreach ($event["EventDate"] as $ed) {
                                    echo date('l, F d, Y', strtotime($ed["date"])) . "  " . date('h:i A', strtotime($ed['start_time'])) . " - " . date('h:i A', strtotime($ed['end_time'])) . "<br>";
                                        ...
                                        <td>
                                            ...
                                            <?php echo $this->html->link('View', PARENT_URL."/Events/viewEvent/".base64_encode($event["Event"]["id"]), array('escape' => false, "class" => "","target"=>"_blank")); ?>
                                            <?php
                                            if($event['Event']['event_from']!="facebook")
                                            {
                                            echo $this->html->link('Edit', PARENT_URL."/Events/editEvent/".base64_encode($event["Event"]["id"]), array('escape' => false, "class" => "","target"=>"_blank"));
                                            }
                                            ?>          
