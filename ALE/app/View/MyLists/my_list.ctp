...
<div class="center-block">
    <div class="repo-list-wrap repo-list-wrapnew">
        <h1>Email Lists</h1>
        <?php echo $this->Session->flash(); ?>
    </div>
    <?php
    echo $this->html->link("Add New List", array("controller" => "MyLists", "action" => "add"), array("class" => "btn-all", "style"=>"float:right", "title" => "Add New List"));
                ...
                if (empty($mylists)) {
                    ...
                    foreach ($mylists as $mylist) {
                        ...
                        <tr>
                            ...
                            <td><?php
                                if ($mylist["MyList"]["total_over_all"] > 0) {
                                    echo round((($mylist["MyList"]["open_mail"] / $mylist["MyList"]["total_over_all"]) * 100), 2) . "%";
                                } else {
                                    echo "0%";
                                }
                                ?></td>
                            <td><?php
                                if ($mylist["MyList"]["total_over_all"] > 0) {
                                    echo round((($mylist["MyList"]["click_mail"] / $mylist["MyList"]["total_over_all"]) * 100), 2) . "%";
                                } else {
                                    echo "0%";
                                }
                                ?></td>
...
//                                echo "&nbsp;" . $this->html->link("CSV", array("controller" => "MyLists", "action" => "exportCsv", base64_encode($mylist["MyList"]["id"])));
                                echo "&nbsp;" . $this->html->link("Add Contact", array("controller" => "MyLists", "action" => "listuserDetail", base64_encode($mylist["MyList"]["id"])), array("title"=>"Add New Contact"));
