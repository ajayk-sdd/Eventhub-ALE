...
<div class="em-sec">
    ...
    <div class="search-panel inner-contentnew">
        ...
        <div id="dateError" style="display: none;text-align: center;font-size: 14px;font-weight: normal">You need to fill/select any of the search critaria.</div>
         
                <label>Find an Event:</label>
                <?php
                echo $this->Form->input("Event.title", array('type' => 'text', 'div' => false, 'label' => false, "placeholder" => "Event Title"));
                ?>
               <input type="submit" id="search-sub-btn" class="src-btn" value="Search">

            </div>
              <?php echo $this->Html->link($txt, 'javascript:void(0);', array("class"=>"bn-hide-show")); ?>
        ...
        <div class="sp-inner">
            <ul class="sp-hide-content <?php echo $clss; ?>" >
                ...
                <li>
                    ...
                    <?php echo $this->Html->link("Change Location", 'javascript:changeLocOpen();', array("class"=>"show-more-location","id"=>"btn_zipfilter")); ?>
                    ...
                    <div class="location-more-option">
                        ...
                        <div style="margin-right:12px"><input type="submit" value="" style="background-color:#ccc;" class="search_button" onclick="javascript:document.search_form.submit();"></div>
    ...
    if(!empty($readAddedEvent))
    {
        ...
        foreach ($readAddedEvent as $key => $value) {
            ...
            <li>
                ...
                <a href="javascript:void(0);" onclick='javascript:selectThisEvent(<?php echo $key; ?>, "<?php echo base64_encode($value["title"]); ?>","<?php echo $value["title"]; ?>");' >
                    Remove
                </a>
            </li>
            <?php
                ...
                foreach ($events as $event) {
                    ...
                    <li>
                        <section class="rcmnded_events" id="<?php echo 'event_box_' . $event['Event']['id']; ?>">
                            ...
                            <?php
                            $viewDetails = PARENT_URL . '/Events/viewEvent/' . base64_encode($event["Event"]["id"]);
                            if ($event["Event"]["event_from"] == "facebook") {
                                ...
                                $iconTitle = "ALH Event";
                                $viewDetails = PARENT_URL . '/Events/viewEventfulEvent/' . $event["Event"]["ef_event_id"];
                            } else {
                                $imgPath = PARENT_URL . "/img/EventImages/small/";
                                $event_icon = 'listhub_icon.png';
                                $iconTitle = "ALH Event";
                            }
                            ?>

                            <a href="javascript:void(0);" class="listhub_icon" title="<?php echo $iconTitle; ?>"><img src="<?php echo PARENT_URL . "/img/" . $event_icon; ?>" alt=""/></a>
                            <?php
                            if (isset($event["Giveaway"]) && $event["Giveaway"]["id"] != '') {
                                ?> 
                                <a href="javascript:void(0);" class="listhub_icon_camp3" title="Ticket Giveaway"><img src="<?PHP echo PARENT_URL; ?>/img/ticon.png" alt=""/></a>
                            <?php }
                            ?>
                            <?php
                            ...
                            <span class="loader" id="load_<?php echo $event['Event']['id']; ?>" style="display: none;"><img src="<?PHP echo PARENT_URL; ?>/img/admin/loader.gif" alt=""></span>
                            ...
                            <section class="events_info">
                                ...
                                <?php
                                 if (strlen($event["Event"]['short_description']) > 130) {
                                            echo substr(strip_tags($event["Event"]['short_description']), 0, 160) . '...';
                                        } else {
                                            echo strip_tags($event["Event"]['short_description']);
                                        }
                                 ?></div></div>
                            </section>
                            <?php
                            ...
                            if (!empty($event["Event"]["ef_event_id"])) {
                                $event_class = "class_" . str_replace("@", "_", $event["Event"]["ef_event_id"]);
                            } else {
                                $event_class = "class_" . $event["Event"]["id"];
                            }
                            ?>
                            <a href="javascript:void(0);" class="btn-addEvent <?php echo $event_class; ?>" id = '<?php echo $event["Event"]["id"]; ?>' title='<?php echo $event["Event"]["title"]; ?>' onclick='selectThisEvent(this.id, "<?php echo base64_encode($event["Event"]["title"]); ?>","<?php echo $event_class;?>");'><?php echo $addedText; ?></a>                            
                        ...
                        <section class="rcmnded_events">
                            ...
                            <section class="events_info">
                                <h3>
                                    ...
                                    $viewDetailsEF = PARENT_URL . '/Events/viewEventfulEvent/' . $evnt_id;
                                    echo $this->html->link($title, $viewDetailsEF, array("class" => "evnt-ttl", "target" => "_blank", "title" => $event['title']));
                                    ?>
                                ...
                                if (isset($EFAddedEventId)) {
                                    if (in_array($event['@attributes']['id'], $EFAddedEventId)) {
                                        $addedText = "- Remove Event";
                                    } else {
                                        $addedText = "+ Add Event";
                                    }
                                    ...
                                    $event_class = "class_" . str_replace("@", "_", $evnt_id);
                                
                                ?>
                                <a href="javascript:void(0);" class="btn-addEvent <?php echo $event_class;?>" id = '<?php echo $evnt_id; ?>' title='<?php echo $title; ?>' onclick='selectThisEFEvent(this.id,"<?php echo $event_class;?>");'><?php echo $addedText; ?></a>                            
...
<script>
    ...
    function selectThisEFEvent(id,classi) {
        $('#load_' + id).show();
        jQuery.ajax({
            url: '/Campaigns/selectMultipleEFEvent/' + id + '/' + classi,
            success: function(data) {
                $('#load_' + id).hide();
                var html = $("#" + id).html();
                if (html == "+ Add Event") {
                    $("." + classi).html("- Remove Event");
                } else {
                    $("." + classi).html("+ Add Event");
                }
    ...
    function selectThisEvent(id, title,classi) {
        $('#load_' + id).show();
        jQuery.ajax({
            url: '/Campaigns/selectMultipleEvent/' + id + '/' + title + '/' + classi,
            success: function(data) {
                $('#load_' + id).hide();
                var html = $("#" + id).html();
                if (html == "+ Add Event") {
    ...
    $(document).ready(function() {
        ...
        if($('.vibes-more-option input[type=checkbox]:checked').length == 0 && $('.categories-more-option input[type=checkbox]:checked').length == 0 && EventTitle=="")
        {
           $("#dateError").show();
        ...
        $('.show-more-location').click(function() {
         ...
         $('.btn-zipfilter-cancel').click(function() {
            $('.location-more-option').removeClass('show-more-option');
        });
